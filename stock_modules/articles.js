...
const publisherBlacklist = ["MONReport","Invest Tribune","StreetInsider.com"];
const options = {
    extras: {
        'down': -1,
        'up': 1,
...
exports.updateArticles = function(){
    ...
    let stocktableFormat = '(id int AUTO_INCREMENT, ticker varchar(8), date varchar(25), title varchar(250), url varchar(250), sentiment float, publisher varchar(40), PRIMARY KEY (id), FOREIGN KEY (ticker) REFERENCES stocktable (ticker));';
                ...
                instance.search(result[0].fullName).then(articles => {
                    ...
                    for (let a = 0; a < maxcount; a++) {
                        if(publisherBlacklist.includes(articles[a].publisher)){
                            ...
                            db.c.query('INSERT INTO '+stocktableName+' (ticker, date, title, url, sentiment, publisher) VALUES (\"'+stocksinConfig[Category][x]+'\", \"'+time+
                                '\", \"'+articles[a].title.replace("'","")+'\", \"'+articles[a].link+'\", '+sentiment.analyze(articles[a].title, options).score+', \"'+articles[a].publisher+'\");');
